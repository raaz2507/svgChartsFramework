...
function LabelsForWeek(){
	...
	for (let i=6 ; i>0; i--){
		...
		const dayNameEn = pastDate.toLocaleDateString('en-US', {weekday : 'short'});
		// const dayNameHi = pastDate.toLocaleDateString('hi-IN', {weekday : 'long'});

		// console.log(dayNameEn, dayNameHi);
		labels.push(`${dayNameEn}`);// ${dayNameHi});
	}
	return labels;
}
